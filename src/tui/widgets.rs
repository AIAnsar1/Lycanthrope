...
fn draw_header(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header = Paragraph::new(Line::from(vec![
        Span::styled(
            " 🐺 LYCANTHROPE ",
            Style::default().fg(CLR_BANNER).add_modifier(Modifier::BOLD),
        ),
        Span::raw("│ "),
        ...
        Span::styled(
            format!("IN:{}", in_count),
            Style::default().fg(CLR_INCOMING),
        ),
        Span::raw(" "),
        Span::styled(
            format!("OUT:{}", out_count),
            Style::default().fg(CLR_INJECTED),
        ),
        Span::raw(" │ "),
        Span::styled(
            format!("Total: {}", app.packets.len()),
    ...
    let items: Vec<ListItem> = app
        ...
        .map(|pkt| {
            ...
            let line = Line::from(vec![
                Span::styled(
                    pkt.timestamp.format("%H:%M:%S ").to_string(),
    ...
    let list = List::new(items)
        .block(
            Block::default()
                ...
                .border_style(Style::default().fg(CLR_BORDER))
                .title(" Packets ")
                .title_style(Style::default().fg(CLR_TITLE)),
        )
        .highlight_style(
            Style::default()
                .bg(Color::DarkGray)
...
fn draw_status_log(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let log_text: String = app.status_log[start..]
        .iter()
        .map(|s| s.as_str())
        .collect::<Vec<_>>()
        .join("\n");
    let log_widget = Paragraph::new(log_text)
        .style(Style::default().fg(CLR_STATUS))
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(CLR_BORDER))
...
fn draw_input(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let input_widget = Paragraph::new(input_text)
        .style(Style::default().fg(CLR_INPUT))
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(CLR_BORDER))
                .title(" Inject Command (Enter to send) ")
                .title_style(Style::default().fg(CLR_TITLE)),
        );
